...
namespace gradution
{
    public partial class form_register_camp : Form
    {
        public form_register_camp()
        {
            InitializeComponent();
        }
        ...
        void display()
        {
            ...
            adp.SelectCommand.CommandText = "Select id_camp,name_c,place_c,count_c,date_start_c,date_finish_c,comment_c,majer_c from Camps  where datediff(day,date_start_c,getdate())<=0";
            adp.Fill(ds, "Camps");
        ...
        private void form_register_camp_Load(object sender, EventArgs e)
        {
            display();
        }
        ...
        private void btn_register_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = "Select CaID, GID  from JoiningCamp where CaID=@M AND GID=@G ";
            cmd.Parameters.AddWithValue("@M", x);
            cmd.Parameters.AddWithValue("@G", y);
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                MessageBox.Show("فارغ التحصیل این اردو را قبلا ثبت نام کرده است");
            ...
            else
            {
                ...
                cmd.Connection = con;
                cmd.CommandText = "insert into JoiningCamp ( CaID, GID)values(@M,@G)";
                cmd.Parameters.AddWithValue("@M", x);
                cmd.Parameters.AddWithValue("@G", y);
                cmd.ExecuteNonQuery();
                disconnect();
                display();
                MessageBox.Show(" ثبت نام فارغ التحصیل در این اردو با موفقیت انجام شد");
            }
            disconnect();
        }
