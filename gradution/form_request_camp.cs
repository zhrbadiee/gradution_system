...
namespace gradution
{
    public partial class form_request_camp : Form
    {
        public form_request_camp()
        {
            InitializeComponent();
        }
        ...
        void display()
        {
            ...
            adp.SelectCommand.CommandText = "Select id_camp,name_c,place_c from Camps ";
            adp.Fill(ds, "Camps");
            ...
            datagrid_list_camp.Columns[2].HeaderText = "مکان اردو";

            datagrid_list_camp.Columns[0].Width = 100;
            datagrid_list_camp.Columns[1].Width = 178;
            datagrid_list_camp.Columns[2].Width = 184;
           
        }

        private void form_request_camp_Load(object sender, EventArgs e)
        {
            display();
        }
        ...
        private void btn_register_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = "Select CaID, GID  from RequestCamp where CaID=@M AND GID=@G ";
            cmd.Parameters.AddWithValue("@M", x);
            cmd.Parameters.AddWithValue("@G", y);
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                MessageBox.Show("فارغ التحصیل برای این اردو قبلا درخواست داده است");
            ...
            else
            {
                ...
                cmd.Connection = con;
                cmd.CommandText = "insert into RequestCamp ( CaID, GID)values(@M,@G)";
                cmd.Parameters.AddWithValue("@M", x);
                cmd.Parameters.AddWithValue("@G", y);
                cmd.ExecuteNonQuery();
                disconnect();
                display();
                MessageBox.Show(" درخواست  فارغ التحصیل در این اردو با موفقیت انجام شد");
            }
            disconnect();
        }
